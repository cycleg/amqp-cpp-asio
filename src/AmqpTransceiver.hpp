...
/// "autodelete" и "durable". При установленном "autodelete" точка обмена
/// существует по крайней мере до первого связывания (binding) ее с очередью,
/// и будет удалена брокером, когда не останется ни одной связи. Флаг "durable"
/// обеспечивает сохранность точки обмена в случае перезапуска брокера.
///
...
/// ничего.
///
/// Автономное использование экземпляров Transceiver невозможно. Пользуйтесь
/// средствами класса Connector, предназначенными для манипуляций
/// приемопередатчиками. К таковым относятся: создание, запуск, остановка и
/// удаление экземпляров Transceiver.
///
...
class Transceiver
{
  template<typename TransceiverImpl> friend class Connector;
  ...
  public:
    ...
    ///
    /// Имя точки обмена, с которой работает приемопередатчик.
    ///
    /// @return Точка обмена.
    ///
    inline std::string exchange_point() const { return m_exchange; }
    ///
    /// Маршрут входящих сообщений для приемника.
    ///
    /// @return Маршрут (routing key).
    ///
    /// Если работает только передатчик, маршрут игнорируется и может быть
    /// пустым.
    ///
    ...
    ///
    /// Запуск, остановка и сброс приемопередатчика производятся исключительно
    /// классом amqp::Connector.
    ///
    /// Запуск осуществляется методом start(). Автомат работает до остановки
    ...
    /// @return Текст ошибки.
    ///
    /// Если конечный автомат данного экземпляра завершился без ошибок,
